<template>
  <Layout v-if="propsbusiness != null">
    ...
    <v-container grid-list-lg>
          ...
          <v-card elevation="5">
            ...
            <v-container pt-0>
              <v-layout row text-center>
                <v-flex xs12> Mail: {{ propsbusiness.contactEmail }} </v-flex>
                <v-flex xs12> Telefon: {{ propsbusiness.contactTel }} </v-flex>
              </v-layout>
            </v-container>
          </v-card>
        </v-flex>
          ...
          <v-card elevation="5">
            ...
            <v-container pt-0>
              <v-layout row text-center>
                ...
                <v-flex xs12>
                  {{ propsbusiness.address.zipCode }}
                  {{ propsbusiness.address.city }}
                </v-flex>
              </v-layout>
            </v-container>
          </v-card>
            ...
            <v-container>
              <v-layout row text-center>
                <v-flex
                  xs12
                  md3
                  v-for="(object, index) in propsbusiness.businessObjects"
                  :key="index"
                >
      ...
      <v-btn block color="blue-grey" @click="displayBusinessEditor" dark
        >Betrieb bearbeiten
      </v-btn>
      <v-btn block color="blue-grey" @click="createTimestamp" dark
        >Timestamp erzeugen
      </v-btn>
      <v-layout row>
        <v-flex>
            ...
            >
              <template v-slot:items="items">
                <tr @click="displaySingleRegistrationForm(items.item)">
                  ...
                  <td>
                    {{
                      $moment(items.item.formData.arrivalDate).format(
                        "DD.MM.YYYY"
                      )
                    }}
                  </td>
                  <td>
                    {{
                      $moment(items.item.formData.departureDate).format(
                        "DD.MM.YYYY"
                      )
                    }}
                  </td>
...
export default {
  ...
  beforeMount() {
    if (typeof this.propsbusiness != "undefined") {
      this.title =
        "Betrieb von " +
        this.propsbusiness.contactPersonName +
        " " +
        this.propsbusiness.contactPersonSurname;
  ...
  methods: {
    getForms() {
      ...
      items = items.filter(
        (form) =>
          parseInt(form.meta.businessObject.business.id) ===
          parseInt(this.propsbusiness.businessId)
      );
      console.log(items);
    ...
    displayBusinessEditor() {
      this.$router.push({
        name: "EditBusiness",
        params: { businessProp: this.propsbusiness },
      });
    },
    createTimestamp() {
      let data = {
        comment: "test",
        hash:
          "2c7d36be542f8f0e7345d77753a5d7ea61a443ba6a9a86bb060332ad56dba38e",
      };
      axios({
        method: 'post',
        url: 'https://api.originstamp.com/v3/timestamp/create',
        headers: {'Authorization': 'bc190d29-702f-480f-b6b1-55c0e20e24ad'},
        data
    })
  }
  }
